...
const CadastroColaboradores = () => {
  ...
  const openEdit = (c) => {
    ...
    setFormData({
      ...
      cargo: c.cargo || "",
      setor: c.setor || "",
      setorDepartamento: c.setorDepartamento || "",
    })
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing
        ? `${API_URL}/api/colaboradores/${editingId}`
      ...
      // se editar, atualizar item na lista, senão adicionar
      if (isEditing) {
        setColaboradores((prev) =>
          prev.map((p) => (p.id === editingId ? { ...p, ...saved } : p))
        )
      } else {
        setColaboradores((prev) => [saved, ...prev])
      }
  ...
  const handleDelete = async (id) => {
    if (!confirm("Deseja excluir esse colaborador?")) return
    try {
          ...
          <div className="flex gap-4">
            ...
            <div className="bg-white rounded-2xl border shadow p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{empresas}</p>
              </div>
            </div>
          </div>
